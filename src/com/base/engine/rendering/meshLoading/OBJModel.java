...
public class OBJModel 
{
	...
	public IndexedModel toIndexedModel()
	{
		...
		for(int i = 0; i < indices.size(); ++i)
		{
			...
			Vector2f currentText;
			Vector3f currentNormal;
			
			if(hasTexCoords)
				currentText = texCoords.get(currentIndex.texCoordIndex);
			else
				 currentText = new Vector2f(0,0);
			...
			if(indexTest != null)
				modelVertIndex = indexTest.intValue();
			...
			if(normalModelIndex == null)
			{
				...
				normalModel.getTexCoords().add(currentText);
				normalModel.getNormals().add(currentNormal);
				normalModel.getTangents().add(new Vector3f(0,0,0));
			}
			
			result.getIndices().add(modelVertIndex);
			normalModel.getIndices().add(normalModelIndex);
			indexMap.put(modelVertIndex, normalModelIndex);
		}
